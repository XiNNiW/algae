...
TEST(Filter_Test, CORE_reson_bandpass) {
  ...
  ResonantBandpass2ndOrderIIR<double, double> filter;
  ...
  algae::dsp::block::mult(0.125, input, BLOCKSIZE, input);
...
TEST(Filter_Test, CORE_resonant_bandpass_amplitude_response) {
  ...
  std::array<double, NUM_BINS> amp_response = compute_amplitude_response<
      double, ResonantBandpass2ndOrderIIR<double, double>, NUM_BINS>(
      filter, SAMPLE_RATE);
  ...
  std::array<double, NUM_BINS> expected_response = {
      2.19633146247128,  2.11374826750154,  1.90320780251077,
      1.64071959156745,  1.38632300758342,  1.16728391682754,
      0.988534736440327, 0.84546251500083,  0.731101846247929,
      0.638981337598576, 0.563884337014535, 0.501832601823901,
      0.449855255264027, 0.405744287577019, 0.367853378511164,
      0.334946364948773, 0.306087214636378, 0.280561303135178,
      0.257819334915452, 0.237437391205991, 0.219088437298083,
      0.202522014218178, 0.187549821998227, 0.174035561488107,
      0.161887811908895, 0.151054931107953, 0.141521007917291,
      0.133301811641021, 0.126439560788983, 0.12099535601983,
      0.11703857686261,  0.114633677119464};
  for (size_t bin_idx = 0; bin_idx < NUM_BINS; bin_idx++) {
    // std::cout << expected_response[bin_idx] << " , " << amp_response[bin_idx]
    // << std::endl;
