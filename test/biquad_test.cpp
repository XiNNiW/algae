...
TEST(Filter_Test, CORE_biquad_lowpass_does_not_explode_FLOAT) {
  ...
  Biquad<float, float> filter;
  filter.lowpass(220, 0.5, 48000);

  algae::dsp::oscillator::WhiteNoise<float> noiseSource;
  noiseSource.process(BLOCKSIZE, input);
...
TEST(Filter_Test, CORE_biquad_lowpass_amplitude_response) {
  ...
  Biquad<double, double> filter;
  filter.lowpass(1000, 0.5, SAMPLE_RATE);

  std::array<double, NUM_BINS> amp_response =
      compute_amplitude_response<double, Biquad<double, double>, NUM_BINS>(
          filter, SAMPLE_RATE);
  ...
  std::array<double, NUM_BINS> expected_response = {
      0.999788611441399,     0.640264937441577,    0.30693822366928,
      0.163351063962347,     0.097943931621367,    0.064081512075196,
      0.04460345660056,      0.032466375762941,    0.024427142959851,
      0.018842657595971,     0.014813768273712,    0.011816745808295,
      0.009530510691689,     0.00774966081001,     0.006337999663849,
      0.005202419866909,     0.004277579057314,    0.00351656731614,
      0.002885036302516,     0.002357399608343,    0.001914311199764,
      0.001540954481378,     0.001225858319814,    0.000960063356335,
      0.000736525979128,     0.000549686638011,    0.000395153880667,
      0.000269471340667,     0.000169945290733,    0.000094517314107658,
      0.0000416713831490537, 0.0000103679451089572};
...
TEST(Filter_Test, CORE_biquad_highpass_amplitude_response) {
  ...
  Biquad<double, double> filter;
  filter.highpass(2000, 0.5, 48000);
  ...
  // response based on coefficients
  std::array<double, NUM_BINS> expected_response = {
      3.69405810764026E-05, 0.122186789148331, 0.358813204211454,
      0.559361515152099,    0.695376093447049, 0.783550844008875,
      0.841500097424935,    0.880762285459868, 0.908250497076891,
      0.928091881739818,    0.942805861101011, 0.95397658160031,
      0.962630754972054,    0.969453143210067, 0.974912571929108,
      0.979337701443556,    0.982963862613871, 0.985962774842868,
      0.988461866396189,    0.990557118551635, 0.992321780285165,
      0.993812391777296,    0.995073018195025, 0.996138270592429,
      0.997035490183264,    0.997786345732512, 0.998408012479269,
      0.998914047742275,    0.999315042853078, 0.999619106959776,
      0.999832221562762,    0.999958492833479};
  for (size_t bin_idx = 0; bin_idx < NUM_BINS; bin_idx++) {
    EXPECT_NEAR(expected_response[bin_idx], amp_response[bin_idx], 0.01);
  }
}
...
TEST(Filter_Test, CORE_biquad_bandpass_amplitude_response) {
  ...
  Biquad<double, double> filter;
  filter.bandpass(4000, 0.8, 48000);
  std::array<double, NUM_BINS> amp_response =
      compute_amplitude_response<double, Biquad<double, double>, NUM_BINS>(
          filter, SAMPLE_RATE);

  // expected values computed in octave using octave freqz function to evaluate
  // filter response based on coefficients
  std::array<double, NUM_BINS> expected_response = {0,
                                                    0.230753381267204,
                                                    0.469155819928966,
                                                    0.706328152678762,
                                                    0.900195945718271,
